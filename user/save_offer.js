...
exports.save_offer = async (req, res, next) => {
    let { shopname, offervalue, productid, start_date  } = req.body;
    // Prevent SQL Injection by escaping user inputs
    console.log(req.body);
    let query = 'INSERT INTO offers(shop_name,offer_value,product_id,start_date) VALUES(?, ?, ?, ?)';
    
    db.query(query, [shopname, offervalue, productid, start_date], (error) => {
        if (error) {
            console.log(error);
